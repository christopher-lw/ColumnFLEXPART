...
if __name__ == "__main__":
    ...
    for i, ((lon1, lat1, lon2, lat2), (date1, time1, date2, time2)) in enumerate(zip(coords, times)):
        for j, parts in enumerate(part_nums):
            comment = f"coords:{(lon1, lat1, lon2, lat2)}, height:{height_levels[j]}, time:{(date1, time1, date2, time2)}"
            z1 = height_levels[j]
